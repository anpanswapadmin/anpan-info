...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name anpanswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name anpanswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name anpanswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  anpanswap_RETURN: 'anpanswap_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.anpanswap_RETURN) {
          return p0?.anpanswap?.return > p1?.anpanswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.anpanswap_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.anpanswap_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.anpanswap_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
