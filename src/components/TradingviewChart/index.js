...
const TradingViewChart = ({
  ...
  const previousTheme = usePrevious(lightMode)

  // reset the chart if them switches
  useEffect(() => {
    if (chartCreated && previousTheme !== lightMode) {
      // remove the tooltip element
      let tooltip = document.getElementById('tooltip-id' + type)
      let node = document.getElementById('test-id' + type)
      node.removeChild(tooltip)
  ...
  }, [chartCreated, lightMode, previousTheme, type])
  ...
  useEffect(() => {
    if (!chartCreated && formattedData) {
      ...
      toolTip.className = lightMode ? 'three-line-legend' : 'three-line-legend-dark'
      ...
      function setLastBarText() {
        toolTip.innerHTML =
          ...
          '</div>' +
          '<div>' +
          dayjs().utc().format('MMMM D, YYYY') + ' (UTC)' +
          '</div>'
      }
      setLastBarText()
