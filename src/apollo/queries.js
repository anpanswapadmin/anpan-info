...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "anpanswap/exchange") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const GLOBAL_CHART = gql`
  query anpansDayDatas($startTime: Int!, $skip: Int!) {
    anpansDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query anpansFactories {
      anpansFactories(
...
export const ALL_PAIRS = (block) => {
  ...
  query pairs($skip: Int!) {
    pairs(first: 100, skip: $skip, orderBy: trackedReserveBNB, orderDirection: desc, where: { createdAtBlockNumber_gte: 9941783 }) {
      id
      token0 {
        id
        symbol
...
export const PAIRS_CURRENT = gql`
  query pairs {
    pairs(first: 100, where: { createdAtBlockNumber_gte : 9941783 },  orderBy: trackedReserveBNB, orderDirection: desc) {
      id
    }
  }
`
