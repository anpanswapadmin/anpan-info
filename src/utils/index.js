...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://exchange.anpanswap.finance/#/` +
      (remove ? `remove` : `add`) +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://exchange.anpanswap.finance/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://exchange.anpanswap.finance/#/swap?inputCurrency=${
      token0Address === '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c' ? 'BNB' : token0Address
...
export function getMiningPoolLink(token0Address) {
  return `https://exchange.anpanswap.finance/#/add/BNB/${token0Address}`
}

export function getAnpanswapAppLink(linkVariable) {
  let baseAnpanswapUrl = 'https://exchange.anpanswap.finance/#/add'
  if (!linkVariable) {
    return baseAnpanswapUrl
  }

  return `${baseAnpanswapUrl}/BNB/${linkVariable}`
}
